...
# Users will enter a 5-letter word and the program outputs whether or not the letter is in the correct position/exists
# Six attempts are given to guess the correct word
...
traverseArray:
	li $s6, 0	#reset number of attempts when replaying
...
checkNumOfCharsloop: 
    	...
    	j checkNumOfCharsloop

#checks if user guesses entire word correctly
checkEntireWord:
    	move $t7, $s2	#$t7 stores address of correct word
    	la $t0, guessBuffer
...
#check if user input is more than 5 letters
checkIfLong: 
    	addi $t0, $t0, 1	#move to next byte/char in word
...
#core loop to compare each letter in user's guess to correct word
loopForGame:
     	lb $t5, 0($t0)	#load byte/char from user guess into $t5 
     	lb $t6, 0($t3)	#load byte/char from correct randomized word into $t6
     	beq $t2, $t4, checkEntireWord	#if byte/char position reaches more than 5-letters (6), then done checking each letter so jump to check entire word
     	...
     	#if none of the conditions above are met, the letter matches
...
#print that the letter is at wrong position
wrongCharAtPosition:
	li $v0, 11	# 11 = print character
	...
	#temp variable to save address of user word again safely
   	...
   	li $t8, 0	#counter
   	li $t9, 5	#max index (5-letters, 0-5)
   
#check if user guess letter exists in correct word at all
checkIfExistsLoop:
   	lb $t1, 0($t7)	#load byte/char of correct word into $t1
   	beq $t8, $t9, doesNotExist	#if we checked all 5-letters (counter equals max index), there is no match so letter does not exist
   	beq $t5, $t1, printExists	#if guessed letter equals a letter in the correct word, the letter exists but wrong position
...
doesNotExist:
   	...
   	j loopForGame	#jump back to core game loop to continue checking each letter
...
#guessed whole word incorrectly 
incorrectGuess:
